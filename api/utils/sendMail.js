...
export const mailSend = async (to, sub, data = {}) => {
  // <!-- create transporter -->
  ...
  await transporter.sendMail({
    from: `"Account Verification" <${process.env.USER_EMAIL}>`, // sender address
...
// <!-- User verification in admin panel -->
export const userMailSendByAdmin = async ({ to, sub, msg }) => {
  // <!-- create transporter -->
  const transporter = nodemailer.createTransport({
    host: process.env.EMAIL_HOST,
    port: process.env.Email_Port,
  ...
  //   <!-- send mail -->
  await transporter.sendMail({
    from: `"Woolmart" <${process.env.USER_EMAIL}>`, // sender address
    to: to, // list of receivers
    subject: sub, // Subject line
    text: msg
    // html: `` // html body
